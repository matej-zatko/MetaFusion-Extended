...
outfile=$outdir/$(basename $cluster).preds.collected
echo $outfile
...
#add header
#echo \#gene1  gene2   chr1    breakpoint_1    chr2    breakpoint_2    max_split_cnt   max_span_cnt    sample_type disease tools   inferred_fusion_type    samples cancer_db_hits  fusion_IDs num_tools   frame   strand1 strand2 | sed 's/ \+/\t/g' > $outdir/$(basename $cluster).TP
header=$(head -1 $cluster)
echo $header | sed 's/ \+/\t/g' > $outdir/$(basename $cluster).TP
...
#echo \#gene1  gene2   chr1    breakpoint_1    chr2    breakpoint_2    max_split_cnt   max_span_cnt    sample_type disease tools   inferred_fusion_type    samples cancer_db_hits  fusion_IDs num_tools   frame   strand1 strand2| sed 's/ \+/\t/g' > $outdir/$(basename $cluster).FP
header=$(head -1 $cluster)
echo $header | sed 's/ \+/\t/g' > $outdir/$(basename $cluster).FP
#echo IDS ${ids[@]}
echo $outdir/$(basename $cluster).FP
for id in ${ids[@]}; do
