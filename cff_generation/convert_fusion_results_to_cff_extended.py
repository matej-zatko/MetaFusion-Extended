...
# Script has been customized to include gene IDs in the CFF file, also support for FusionCatcher was added
...
class FusionResult:
    def __init__(self, tool_name, line, idxes):
        ...
        (idx_chr1, idx_pos1, idx_strand1, idx_chr2, idx_pos2, idx_strand2, idx_frame, idx_split_cnt, idx_pair_cnt,
         idx_gene1, idx_gene2, idx_gene_location1, idx_gene_location2, idx_gene1id, idx_gene2id) = idxes
        self.tool = tool_name
        # DRAGEN
        ...
        # STAR-FUSION NEEDS FIELDS ASSIGNED DIFFERENTLY
            ...
            # assign frame based on whether --examine_coding_effect flag was used
            self.frame = tmp[idx_frame] if idx_frame != "NA" else "NA"
            self.gene1id = tmp[idx_gene1id].split("^")[1].split(".")[0]
            self.gene2id = tmp[idx_gene2id].split("^")[1].split(".")[0]
            ...
            self.gene1id = tmp[idx_gene1id] if idx_gene1id != "NA" else "NA"
            self.gene2id = tmp[idx_gene2id] if idx_gene2id != "NA" else "NA"
            ...
            self.pair_cnt = tmp[idx_pair_cnt] if idx_pair_cnt != "NA" else -1
            self.frame = tmp[idx_frame]
            self.gene1id = tmp[idx_gene1id]
            self.gene2id = tmp[idx_gene2id]
    ...
    def __init__(self, result_file):
        ...
        for line in open(result_file, "r"):
            ...
            if tmp[0] == "#FusionGene" and tmp[1] == "Score":  # STAR-SEQR header
                ...
                self._idx_frame = "NA"
                self._idx_gene1id = "NA"
                self._idx_gene2id = "NA"
                ...
                self._idx_gene1id = tmp.index("gene_id1") if "gene_id1" in tmp else "NA"
                self._idx_gene2id = tmp.index("gene_id2") if "gene_id2" in tmp else "NA"
                ...
                self._idx_gene_location1 = "NA"
                self._idx_gene_location2 = "NA"
                self._idx_gene1id = tmp.index("LeftGene")
                self._idx_gene2id = tmp.index("RightGene")
                try:
                ...
                self._idx_frame = "NA"
                self._idx_gene1id = tmp.index("gene1")
                self._idx_gene2id = tmp.index("gene2")
                ...
                self._idx_gene_location2 = "NA"
                self._idx_frame = "NA"
                self._idx_gene1id = tmp.index("EnsemblGene1")
                self._idx_gene2id = tmp.index("EnsemblGene2")
            ...
            elif tmp[0] == "Gene_1_symbol(5end_fusion_partner)":
                self.tool = "Fusioncatcher"

                self._idx_chr1 = tmp.index("Fusion_point_for_gene_1(5end_fusion_partner)")
                self._idx_chr2 = tmp.index("Fusion_point_for_gene_2(3end_fusion_partner)")
                self._idx_pos1 = tmp.index("Fusion_point_for_gene_1(5end_fusion_partner)")
                self._idx_pos2 = tmp.index("Fusion_point_for_gene_2(3end_fusion_partner)")
                self._idx_strand1 = tmp.index("Fusion_point_for_gene_1(5end_fusion_partner)")
                self._idx_strand2 = tmp.index("Fusion_point_for_gene_2(3end_fusion_partner)")
                self._idx_split_cnt = "NA"
                self._idx_pair_cnt = tmp.index("Spanning_pairs")
                self._idx_gene1 = tmp.index("Gene_1_symbol(5end_fusion_partner)")
                self._idx_gene2 = tmp.index("Gene_2_symbol(3end_fusion_partner)")
                self._idx_gene_location1 = "NA"
                self._idx_gene_location2 = "NA"
                self._idx_frame = tmp.index("Predicted_effect")
                self._idx_gene1id = tmp.index("Gene_1_id(5end_fusion_partner)")
                self._idx_gene2id = tmp.index("Gene_2_id(3end_fusion_partner)")
                ...
                fusion_line = FusionResult(self.tool, line,
                                           [self._idx_chr1, self._idx_pos1, self._idx_strand1, self._idx_chr2,
                                            ...
                                            self._idx_gene_location1, self._idx_gene_location2, self._idx_gene1id, self._idx_gene2id])
...
for fusion in fusion_results:
    print >> out_file, "\t".join(map(str, [fusion.chr1, fusion.pos1, fusion.strand1, fusion.chr2, fusion.pos2,
                                           ...
                                           fusion.gene1, fusion.gene1id, fusion.gene_location1, fusion.gene2, fusion.gene2id, fusion.gene_location2]))
